...
async function requireUser(req, config) {
  ...
  // Backward compatibility: if user_profiles table is not initialized yet,
  // allow logged-in staff users with viewer metadata to operate as admins.
  if (!profileStoreAvailable && effectiveRole === ROLE_VIEWER) {

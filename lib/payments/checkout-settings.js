...
function normalizeShippingProvider(item, index = 0) {
  ...
  const normalizedRaw = normalizeText(rawProvider, 40)
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9_-]/g, '')
  ...
  const provider = aliasMap[normalizedRaw] || normalizedRaw;
...
function normalizeShippingSettings(raw) {
  ...
  providers.forEach((item) => seenProviders.add(item.provider));
  DEFAULT_SHIPPING_SETTINGS.providers.forEach((item, index) => {
    const normalizedItem = normalizeShippingProvider(item, index);
    if (!normalizedItem) return;
    if (seenProviders.has(normalizedItem.provider)) return;
    providers.push(normalizedItem);
    seenProviders.add(normalizedItem.provider);
  });

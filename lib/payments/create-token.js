...
module.exports = async function handler(req, res) {
  ...
  const paytrEnabledFromSettings = checkoutSettings?.payment?.paytr_enabled !== false;
  const usePaytr = forcePaytrMode
    ? missingPaytrEnv.length === 0
    : !forceMockMode && paytrEnabledFromSettings && missingPaytrEnv.length === 0;
  ...
  try {
    ...
    if (!usePaytr) {
      let mockReason = 'paytr_not_configured';
      if (forceMockMode) mockReason = 'mock_mode_forced';
      else if (!paytrEnabledFromSettings) mockReason = 'paytr_disabled_in_settings';
      ...
      return sendJson(res, 200, {
        ...
        account: accountResult.requested
          ...
          : undefined,
        message: !paytrEnabledFromSettings
          ? 'Siparis olusturuldu. PayTR odeme ayarlarda pasif oldugu icin siparisiniz pending olarak kaydedildi.'

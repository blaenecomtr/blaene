...
const { readJsonBody, getClientIp, getRequestPath } = require('../api/request');
const { normalizeText, normalizeEmail } = require('../api/validation');
...
function sanitizeMetadata(body) {
  return {
    ...
    track_kind: normalizeText(body.track_kind, 60) || null,
    product_code: normalizeText(body.product_code, 80) || null,
    product_name: normalizeText(body.product_name, 160) || null,
    customer_email: normalizeEmail(body.customer_email) || null,
...
module.exports = createApiHandler(
  ...
  async (req, res, ctx) => {
    ...
    const customerEmail = normalizeEmail(body.customer_email) || null;
